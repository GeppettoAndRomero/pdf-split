...
export async function registerServiceWorker(): Promise<ServiceWorkerRegistration | null> {
  ...
  try {
    // Service Workerを登録（/pdf-split/sw.js, scope=/pdf-split/）。
    // scope がツールの全 locale ページ（/pdf-split/, /pdf-split/ja/ …）を被覆する。
    const registration = await navigator.serviceWorker.register('/pdf-split/sw.js', {
      scope: '/pdf-split/',
    });
